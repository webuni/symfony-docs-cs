...
Služba ``database_connection`` vždy vrací výchozí (*default*) připojení,
which is the first one defined or the one configured via the
``default_connection`` parameter.

Ke každému připojení je přiřazena služba jejíž název je ve tvaru ``doctrine.dbal.[name]_connection``,
kde ``[name]`` je název připojení.
